...
func (s *JsonPrettifierSuite) TestPrettifyEntry(c *C) {
	...
	// The line number of below line is 37 which will be used as value of 'Line' field in record
	record := NewRecord(LOG_INFO, "Hello, world", map[string]string{"foo": "bar"})
	...
	// One example:
	// INFO Wed, 19 Sep 2012 10:51:57 CST json_prettifier_test.go:37:TestPrettifyEntry {"foo":"bar"} Hello, world
	c.Assert(string(bytes), Matches, `INFO .*son_prettifier_test.go:37:TestPrettifyEntry.*{"foo":"bar"}.*Hello, world`)
}
